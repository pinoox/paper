  ...
  <section>
    <ch-drawer custom-class="drawer-wrapper"
      ...
      <div class="drawer-content">
        <row :gutter="12" :columns="2">
          <column :sm="2" :md="1">
            ...
            <div class="input-wrapper" v-if="!!params.schedule_date">
              <label class="input-label">{{ LANG.post.release_schedule }}</label>
              <span class="schedule-alert datetime-show">{{ params.schedule_date }}</span>
            </div>
          </column>
          <column :sm="2" :md="1">
            <div v-if="$parent.post_type==='page'" class="input-wrapper">
      ...
      <div slot='footer' class="drawer-footer" v-else>
        ...
        <div class="btn btn-warning" @click="cancelScheduling()" v-if="$parent.status === 'schedule'">
          {{ LANG.post.cancel_scheduling }}
          <span class="datetime-show">({{ params.schedule_date }})</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    cancelScheduling() {
      this.$parent.enableHook();
      this.$parent.save('draft').then(() => {
        this.params.schedule_date = null;
        this.$parent.params.schedule_date = null;
      });
    },
    openDatePicker() {
    ...
    getParams() {
      return {
        ...
        schedule_date: this.$parent.params.schedule_date,
    ...
    saveParams() {
      ...
      this.$parent.params.schedule_date = this.params.schedule_date;
      this.$parent.params.tags = this._clone(this.params.tags);
      this.$parent.params.image = this._clone(this.params.image);
      let status = null;

      if (this.params.schedule_date !== paramsParent.schedule_date)
        status = 'schedule';
      else if (this.params.title !== paramsParent.editor.title)

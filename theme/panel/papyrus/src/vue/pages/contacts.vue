            ...
            <vue-good-table
              ...
              <template slot="table-row" slot-scope="props">
                ...
                <div v-else-if="props.column.field === 'operation'">
                  <span v-if="props.row.status === 'unseen'" @click="changeStatus('seen',props.row,props.index)" class="btn-action"><i
                      class="fa fa-eye"></i></span>
                  <span v-if="props.row.status === 'seen'" @click="changeStatus('unseen',props.row,props.index)" class="btn-action"><i
...
export default {
  ...
  methods: {
    ...
    rowStyleClassFn(row) {
      return row.status === 'unseen'? 'bg-color-unseen' : '';
    },
    changeStatus(status, row, index) {
      this.$http.post(this.URL.API + 'contact/changeStatus/', {
